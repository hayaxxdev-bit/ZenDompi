...
export function WalletList({ wallets, isLoading, userId }: WalletListProps) {
  ...
  // Mapping tipe wallet ke warna & icon
  ...
  if (wallets.length === 0) {
    return (
      <Card>
        ...
        {/* Tampilkan form hanya jika tombol Tambah Dompet diklik */}
        {userId && showAddForm && (
          <div className="mt-4 pt-4 border-t border-zinc-800/50 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-zinc-400">Tambah Dompet Baru</span>
              <button
  ...
  return (
    <Card>
        ...
        action={
          userId && (
            <button
              ...
              title={showAddForm ? "Tutup" : "Tambah Dompet"}
            >
        ...
        {wallets.map((wallet) => (
          <div
            ...
            className={`rounded-lg border-l-2 p-3 ${
              typeColors[wallet.type] || "border-l-zinc-600 bg-zinc-800/30"
            }`}
          >
            <div className="flex items-center justify-between">
      ...
      {/* Form Tambah Dompet ketika tombol Plus / Tambah diklik */}
      {userId && showAddForm && (
